...
const QueryUI = ({
  ...
  const runQuery = async (e) => {
    e.preventDefault();
    bindingStream.current = await queryEngine.current.queryBindings(
      query,
      {
        sources: [queryUrl],
        fetch: session.fetch
  ...
  return (
    <div>
      <form onSubmit={runQuery}>
        ...
        <br/>
        <label htmlFor="root">Root URL: 
        <input id="root" type='text' value={queryUrl} onChange={
          (e) => setQueryUrl(e.target.value)
        } />
        </label>
        <br/>
